...
ICudaEngine* createEngineSmall(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt)
{
	...
	std::map<std::string, Weights> weightMap = loadWeights("E:\\ALGXML\\model.wts");
	Weights emptywts{ DataType::kFLOAT, nullptr, 0 };
	...
	IFullyConnectedLayer* fc1 = network->addFullyConnected(*sw1->getOutput(0), 100, weightMap["classifier.0.weight"], weightMap["classifier.0.bias"]);
	assert(fc1);
	...
	IFullyConnectedLayer* fc2 = network->addFullyConnected(*sw2->getOutput(0), 2, weightMap["classifier.3.weight"], weightMap["classifier.3.bias"]);
...
ICudaEngine* createEngineLarge(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt)
{
	...
	std::map<std::string, Weights> weightMap = loadWeights("../model.wts");
	Weights emptywts{ DataType::kFLOAT, nullptr, 0 };
...
int main(int argc, char** argv)
{
	...
	size_t size{ 0 };
	std::string mode = "small"/*std::string(argv[2])*/;
	std::cout << mode << std::endl;
	if (true/*std::string(argv[1]) == "-s"*/) {
		std::cout << "1.0" << std::endl;
		IHostMemory* modelStream{ nullptr };
		APIToModel(1, &modelStream, mode);
		assert(modelStream != nullptr);
		std::ofstream p("BrokenModel_" + mode + ".engine", std::ios::binary);
		if (!p) {
